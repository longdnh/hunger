CREATE TABLE users (
    ...
    CONSTRAINT chk_users_user_id_uuidv7
...
CREATE TABLE tenant (
    ...
    CONSTRAINT chk_tenant_tenant_id_uuidv7
        CHECK (uuid_extract_version(tenant_id) = 7),
    CONSTRAINT chk_tenant_name_not_blank
        CHECK (btrim(name) <> ''),
    CONSTRAINT chk_tenant_code_not_blank
        CHECK (btrim(tenant_code) <> ''),
    CONSTRAINT chk_tenant_timestamps
        CHECK (updated_at >= created_at)
);
...
CREATE TABLE membership (
    ...
    CONSTRAINT chk_membership_timestamps
        CHECK (updated_at >= created_at)
);

CREATE INDEX idx_membership_tenant_id
...
CREATE TABLE user_activation_tokens (
    ...
    CONSTRAINT pk_user_activation_tokens
        PRIMARY KEY (user_activation_token_id),
...
CREATE TABLE user_sessions (
    ...
    CONSTRAINT chk_user_sessions_token_hash_not_blank
        CHECK (btrim(token_hash) <> '')
);
